...
globalThis.bytebeat = new class {
	...
	get timeCursorEnabled() {
		return this.songData.sampleRate >> this.settings.drawScale < 40;
	}
	animationFrame() {
	...
	async init() {
		...
		if (!window.location.hostname.includes`gfljbeat2`) { // You may also change this line to your hostname. (instead of `chasyxx`, use `t-8492` for example)
	...
	setCodeSize(value) {
		this.controlCodeSize.textContent = `${this.formatBytes(new Blob([value]).size)} (${new Blob([String(this.formatBytes((window.location))])})`;
	}
